...
A shared snippet with a small, fixed set of parameters and no control flow of its own — pure formatting, like `resource_ref` (`dict "kind" "name" "namespace"(opt) "callerNamespace"(opt) "nameSuffix"(opt)` → a colored `Kind/name -n ns` string) — belongs in `template_functions_static.go` as a Go function registered in the funcMap, not a `{{define}}` block taking an untyped `dict`. A Go signature is compiler-checked (wrong argument count/order is a build failure, not a blank field at render time) and gets ordinary unit test coverage in `template_functions_static_test.go`, the same way `colorPercent` already does.
