...
test: vet staticcheck
	...
	# CGO_ENABLED=1 is required for -race.
	CGO_ENABLED=1 go run $(GOTESTSUM_MODULE) --junitfile unit-junit.xml -- -race -coverprofile=cover.out -coverpkg=./... -covermode=atomic ./...
...
test-e2e: vet staticcheck install-e2e-deps
	...
	@mkdir -p $(E2E_HOME)
	CGO_ENABLED=1 RUN_E2E_TESTS=true ASSUME_CLUSTER_IS_CONFIGURED=true $(E2E_CONTEXT_ENV) flock $(E2E_LOCKFILE) go run $(GOTESTSUM_MODULE) --junitfile e2e-junit.xml -- ./... -count=1 -timeout=40m -parallel=4 -run 'TestE2E*' -race -coverprofile=cover-e2e.out -coverpkg=./... -covermode=atomic
else
test-e2e: vet staticcheck e2e-cluster-up install-e2e-deps
	...
	$(E2E_KUBECONFIG_ENV) CGO_ENABLED=1 RUN_E2E_TESTS=true ASSUME_CLUSTER_IS_CONFIGURED=true $(E2E_CONTEXT_ENV) flock $(E2E_LOCKFILE) go run $(GOTESTSUM_MODULE) --junitfile e2e-junit.xml -- ./... -count=1 -timeout=40m -parallel=4 -run 'TestE2E*' -race -coverprofile=cover-e2e.out -coverpkg=./... -covermode=atomic
endif
